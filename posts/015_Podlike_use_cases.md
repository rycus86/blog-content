...
The use-cases we're going to look at are:
...
The application in the example is configured to write in a log file that happens to sit on the shared volume, and the other instance will simply *tail* them from there to the standard output. It could also just as easily forward them to a central log aggregator.
